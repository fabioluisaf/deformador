...
void Model::computeNormals() {
  // Zerando os vetores normais dos vertices
  for (auto& vertex : m_vertices) {
    vertex.normal = glm::zero<glm::vec3>();
  }

  // Computando o vetor na direcao do normal
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    ...
    // Vetor na direcao do normal
    const auto edge1{b.position - a.position};
    const auto edge2{c.position - b.position};
    glm::vec3 normal{glm::cross(edge1, edge2)};

    // Soma os normais das faces aos dos vertices, isso equivale
    // a fazer a media dos vetores
    a.normal += normal;
    b.normal += normal;
    c.normal += normal;
  }
...
void Model::computeTangents() {
  // Salvando espacos para as bitangentes
  std::vector<glm::vec3> bitangents(m_vertices.size(), glm::vec3(0));

  // Calculando tangentes e bitangentes das faces
  for (const auto offset : iter::range<int>(0, m_indices.size(), 3)) {
    // Indices das faces
    ...
    // Somando as tangentes dos vertices
    v1.tangent += tangent;
    v2.tangent += tangent;
    v3.tangent += tangent;
...
void Model::render(int numTriangles) const {
  ...
  // Parametros da minificacao e da magnificacao
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

  // Parametros do texture wrapping
...
void Model::setupVAO(GLuint program) {
  ...
  GLint offsetAttribute{glGetAttribLocation(program, "inOffset")};
  if (offsetAttribute >= 0) {
    glEnableVertexAttribArray(offsetAttribute);
    GLsizei offset{sizeof(glm::vec3) + sizeof(glm::vec3) + sizeof(glm::vec2) + sizeof(glm::vec4)};
    glVertexAttribPointer(offsetAttribute, 3, GL_FLOAT, GL_FALSE,
                          sizeof(Vertex), reinterpret_cast<void*>(offset));
  }
...
void Model::standardize() {
  // Centralizar o objeto na origem e normalizar para que ele fique entre [-1, 1]
  ...
  // Centralizando e ajustando o tamanho
...
void Model::limpaOffsets() {
  for(auto& vertex : m_vertices) {
    vertex.offset = glm::vec3{0,0,0};
  }
  
  createBuffers();
}

void Model::deformaEspinho(float tamanho) {
  if(!m_hasNormals) {
    computeNormals();
  }

  std::uniform_real_distribution<float> distrib(0, tamanho*2);

  for(auto& vertex : m_vertices) {
    float espinho = distrib(m_rnjesus);

    vertex.offset = glm::vec3{
      vertex.normal.x*espinho,
      vertex.normal.y*espinho,
      vertex.normal.z*espinho
    };
  }
...
void Model::deformaAleatorio(float tamanho, float razao) {
  if(!m_hasNormals) {
    computeNormals();
  }

  std::uniform_real_distribution<float> distrib(0, tamanho*2);
  std::uniform_real_distribution<float> razaoDistrib(0, 1.0f);

  for(auto& vertex : m_vertices) {
    vertex.offset = glm::vec3{0,0,0};

    if(razaoDistrib(m_rnjesus) < razao) {
      float espinho = distrib(m_rnjesus);
...
void Model::deformaOnda(float periodo) {
  if(!m_hasNormals) {
    computeNormals();
  }

  for(auto& vertex : m_vertices) {
    float onda = 0.1*sin(vertex.position.y*timerOnda.elapsed()*periodo);

    vertex.offset = glm::vec3{
      vertex.normal.x*onda,
      vertex.normal.y*onda,
      vertex.normal.z*onda
    };
  }
